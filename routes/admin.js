...
app.get('/dashboard', auth.isLogin, async (req, res) => {
    try {
        ...
        const userData = await User.find({
            is_admin: 0,
            $or: [
                { name: { $regex: ".*" + search + ".*" } }, //to search for all words that have the search keyword 
                { email: { $regex: ".*" + search + ".*" } }, //to search for all emails that have the search keyword 
                { name: {$regex: "^"+search + ".*"}} //to search for all words that start with the search keyword 
            ],
        });
